...
use ast::{
    BinOp, BinOpExpr, Expr, ExprNode, FnCallExpr, Lit, Postfix, PostfixExpr, TypeConsExpr,
...
impl<'a> Generator<'a> {
    pub fn gen_scope_hash_expr(&mut self, scope: &Scope) -> ExprNode {
        ...
        for expr in hash_exprs {
            // acc * 33
            let mul: ExprNode = BinOpExpr::new(
                BinOp::Times,
                acc,
                ExprNode::from(Lit::U32(33)),
            ).into();
    ...
    fn convert_to_u32_hash(&mut self, expr: ExprNode, ty: &DataType) -> ExprNode {
        match ty {
            ...
            DataType::Vector(n, sub_ty) => {
                ...
                let mut acc = PostfixExpr::new(
                    cast_expr.clone(),
                    Postfix::member(components[0])
                ).into();

                for i in 1..*n as usize {
                    let member: ExprNode = PostfixExpr::new(
                        cast_expr.clone(),
                        Postfix::member(components[i])
                    ).into();
